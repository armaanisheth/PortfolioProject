...
The first two ideas interest me more as an implementor. However, I will attempt to design an implementation of the first one involving a music playlist. I believe it is more relevant to clients. Additionally, I feel excited about this topic which will allow for greater creativity later on. From a coding perspective, the playlist should not be very hard to implement. I will use a data structure such as set or map to avoid repeating songs. The structure will store a person's playlist, and it will have the ability to add, remove, and determine the length of the playlist for kernel methods. As additional methods, it could have a shuffle capability, finding a specific song, finding all the songs matching a certain artist, etc. Therefore, keeping in mind these factors, I believe it will be very reasonable to implement this design.
